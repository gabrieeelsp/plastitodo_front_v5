                        ...
                        <v-row>
                            <v-col>
                                ...
                                >
                                    ...
                                    <tbody>
                                        <tr>
                                        <td class="text-right" >{{ globalHelperFixeDecimalCantidad(getStock(item_stock)) }}</td>
                                        <td class="text-right" v-for="sucursal in sucursals" :key="sucursal.id" >
                                            <template v-if="getStockSucursal(item_stock, sucursal.id) != null" >
                                                <span :class="is_stock_low(item_stock, sucursal.id) ? 'red--text': ''">{{ globalHelperFixeDecimalCantidad(getStockSucursal(item_stock, sucursal.id).attributes.stock / item_stock.attributes.relacion_venta_stock) }}</span>
                                            </template>
                                            <template v-else>
  ...
  export default {
    ...
    methods: {
            ...
            if ( this.valid ) {
                if ( this.item.tipo == 'saleproduct' ) {
                    ...
                    this.$emit('addItem', {
                        ...
                        relacion_venta_stock: this.item_stock.attributes.relacion_venta_stock,
                        is_prepared: false,

                        relationships: {
                            stockproduct: {
                                relationships: {
                                    stocksucursals: this.item_stock.relationships.stockproduct.relationships.stocksucursals
                                }
                            }
                        }
                    })
                                    ...
                                    if ( saleproduct.attributes.is_enable ) {
                                        ...
                                        let var_sale = {
                                            ...
                                            relationships: {
                                                stockproduct: {
                                                    relationships: {
                                                        stocksucursals: saleproduct.relationships.stockproduct.relationships.stocksucursals
                                                    }
                                                }
                                            }
                                        }
        ...
        async onload() {
            ...
            if ( this.item.tipo == 'saleproduct' ) {
                await axios.get(`saleproducts/${this.item.id}`)
                    .then((resp) => {
                        this.item_stock = resp.data.data
        ...
        getStock (item) {
            ...
            return stock / item.attributes.relacion_venta_stock
        },
