        ...
        <v-form @submit.prevent="submit" ref="form" v-model="valid"     >
            ...
            <v-row>
                ...
                <v-col cols="12" sm="4"  class=" pt-0 pb-0">
                    <v-checkbox
                        class="mt-1 mb-1"
                        v-model="item_cache.attributes.is_enable"
                        label="Publicado"
                        color="green"
                        hide-details
                        ></v-checkbox>
...
export default {
    ...
    methods: {
        ...mapActions({
            update_item_resumen: 'combos_manager/update_item_resumen',
        }),
        reset () {
            this.item_cache.attributes.name = this.item.attributes.name
            this.item_cache.attributes.is_enable = this.item.attributes.is_enable
        },
        validate () {
            ...
            if ( this.valid ) {
                ...
                await this.update_item_resumen()
                    .then(() => {
                        ...
                        this.item.attributes.is_enable = this.item_cache.attributes.is_enable

                    })
                    .catch((error) => {
                        if ( error.response ){
                            this.$toast.error(error.response.data[1], { timeout: 3000 });
                        }else {
