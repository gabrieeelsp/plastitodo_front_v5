        ...
        <v-row >
            <v-col cols="12" >
                <v-simple-table class="">
                    ...
                    <tbody>
                        <tr
                        v-for="item in creditnote_editing.creditnoteItems"
                        :key="item.id"
                        :class="[(item.is_creditnote_item ) ? 'blue lighten-5': '']"
                        >
                        <td class=" d-flex align-center pl-1 pr-1 pt-0">
                            <v-checkbox
                            class="  mt-0 pt-0   "
                                v-model="item.is_creditnote_item"
                                hide-details=""
                            >                    
                            </v-checkbox>
                        </td>
                        ...
                        <td
                            v-if="item.is_creditnote_item && item.is_editing"
                        >
                            <InputEditValue
                                :item="{
                                    id: item.id,
        ...
        <v-row>
            <v-col>
                ...
                <v-btn
                    ...
                    :loading="is_saving" 
                    @click="submit"
                    :disabled="totalCreditnoteEditing <= 0"
                >Guardar</v-btn>
            </v-col>
        </v-row>
    </div>
...
export default {
    computed: {
        ...mapGetters({
            ...
            totalCreditnoteEditing: 'sales_manager/totalCreditnoteEditing',
		})
    },
    components: {
        InputEditValue
    ...
    methods: {
        set_value (item_resp) {
            for ( let item of this.creditnote_editing.creditnoteItems ) {
